...
function d_createscrapbookdata()
    ...
    local s = io.open("scripts/screens/redux/creature_list.lua", "w")
    local crestr = ""
    ...
    if f then
        ...
        for i, data in pairs(scrapbookprefabs) do
            ...
            if t:HasTag("shadowcreature") or t:HasTag("stalker") then
                subcat = '"Shadow"'
            end
            ...
            if thingtype == "thing" then
                crestr = crestr .. t.prefab .."\n"
            end
           ...
           -- if thingtype ~= "thing" then  -------------------------TEMPORARY
           ...
           exporter_data_helper:write(string.format("[\"%s\"] = 0x%X,\n", i, hash(i)))
            str = str .. string.format('["%s"] = {name="%s", tex="%s.tex", subcat=%s, type="%s", prefab="%s",', i, name, tex, subcat, thingtype, t.prefab)
            ...
            if t.components.sanityaura and getsanity(t) then
                str=str..' sanityaura='..  getsanity(t) ..','
            end

            --------------------------------------
            ---------------------------------- HEALTH
            if t.components.health then
                str=str..' health='..  t.components.health.maxhealth ..','
            end

            --------------------------------------
            ---------------------------------- DAMAGE
            if t.scrapbook_damage then
                str=str..' damage='..  t.scrapbook_damage ..','
            elseif  t.components.combat and t.components.combat.defaultdamage then
                str=str..' damage='..  t.components.combat.defaultdamage ..','
            end            

            --------------------------------------
            ---------------------------------- STACK
            if t.components.stackable  then                
                str=str..' stacksize='..  t.components.stackable.maxsize ..','
            end

            --------------------------------------
            ---------------------------------- FOOD
            if t.components.edible  then                
                str=str..' hungervalue='..  t.components.edible.hungervalue ..','
                str=str..' healthvalue='..  t.components.edible.healthvalue ..','
                str=str..' sanityvalue='..  t.components.edible.sanityvalue ..','
            end

            if t.components.edible and t.components.edible.foodtype  then                
                str=str..' foodtype="'..  t.components.edible.foodtype ..'",'
            end
            ...
            if t.components.weapon or t.scrapbook_weapondamage then
                if t.prefab == "bomb_lunarplant" then
                    ...
                    str=str..' weapondamage='..  t.components.weapon.damage ..','
                    
                    str=str..' planardamage='..  TUNING.BOMB_LUNARPLANT_PLANAR_DAMAGE ..','
                   
                    str=str..' weaponrange='..  t.components.weapon.hitrange ..','
                    ...
                    if t.scrapbook_weapondamage or t.components.weapon and t.components.weapon.damage then
                        ...
                        if not weapondamage and type(t.components.weapon.damage) == "function" then
                            ...
                            str=str..' weapondamage='.. weapondamage ..','
                        end
                    end
                    ...
                    if planardamage then
                        str=str..' planardamage='.. planardamage ..','
                    end
            ...
            if t.components.armor then                
                str=str..' armor='.. t.components.armor.maxcondition ..','
                str=str..' absorb_percent='.. t.components.armor.absorb_percent ..','
            ...
            if t.components.finiteuses  then                
                str=str..' finiteuses='..  t.components.finiteuses.total ..','
            end            
            if t.components.tool then
                str=str..' toolactions={'
            ...
            if t.scrapbook_setanim then
                str=str..' scrapbook_setanim='.. t.scrapbook_setanim ..','
            end

            if t.scrapbook_overridebuild then
                str=str..' scrapbook_overridebuild="'.. t.scrapbook_overridebuild ..'",'
            end

            if t.scrapbook_hide then
                str=str..' scrapbook_hide={'
    ...
    print("PREFAB:",t.prefab,i)
            ...
            str=str..' build="'..  build ..'",'
            str=str..' bank="'..  t.AnimState:GetCurrentBankName() ..'",'
            str=str..' anim="'..  anim ..'",'

            if t.scrapbook_overridedata then
                if type(t.scrapbook_overridedata[1]) ~= "table" then
                    str=str..' overridesymbol={"'..t.scrapbook_overridedata[1]..'","'..t.scrapbook_overridedata[2]..'","'..t.scrapbook_overridedata[3]..'"},'
                else
                    str=str..' overridesymbol={'

                    for od,odset in ipairs(t.scrapbook_overridedata) do
                       str=str.. '{"'.. odset[1] ..'","'.. odset[2] ..'","'.. odset[3] ..'"},'
                    end
            ...
            if t.prefab == "mosquito" then
                str= str..' animoffsety=' .. 100 ..', '
                str= str..' animoffsetx=' .. -20 ..', '
            end 
            if t.prefab == "moon_altar_seed" then
                str= str..' animoffsety=' .. 20 ..', '
                str= str..' animoffsetx=' .. 25 ..', '
            end
            ...
            if t.prefab == "moon_altar_crown" then
                str= str..' animoffsety=' .. -20 ..', '
                str= str..' animoffsetx=' .. 25 ..', '                
                str= str..' animoffsetbgy=' .. 30 ..', '
            end
            if t.prefab == "shroomcake" then
                str= str..' animoffsety=' .. -20 ..', '
                str= str..' animoffsetbgy=' .. 25 ..', '
            end
            if t.prefab == "vegstinger" then
                str= str..' animoffsety=' .. -10 ..', '
            end  
            ...
            if t.prefab == "bernie_active" then
                str= str..' animoffsety=' .. 60 ..', '
                str= str..' animoffsetbgy=' .. -50 ..', '
            end 
            ...
            if t.components.waterproofer then
                str= str..' waterproofer=' .. t.components.waterproofer:GetEffectiveness() ..', '
            end

            -----------------------------------------
            ------------------------------------ INSULATOR
            if t.components.insulator then
                str= str..' insulator=' .. t.components.insulator:GetInsulation() ..', '
            end  

            -----------------------------------------
            ------------------------------------ DAPPERNESS
            if t.components.equippable and t.components.equippable.dapperness then
                str= str..' dapperness=' .. t.components.equippable.dapperness ..', '
            end

            -----------------------------------------
            ------------------------------------ FUELED
            if t.components.fueled then
                str= str..' fueledmax=' .. t.components.fueled.maxfuel ..', '
                str= str..' fueledrate=' .. t.components.fueled.rate ..', '
                str= str..' fueledtype1="' .. t.components.fueled.fueltype ..'", '
                if t.components.fueled.secondaryfueltype then
                    str= str..' fueledtype2="' .. t.components.fueled.secondaryfueltype ..'", '
                end
            end          
            ...
            if t.components.fuel then
                str= str..' fueltype="' .. t.components.fuel.fueltype ..'", '
                str= str..' fuelvalue=' .. t.components.fuel.fuelvalue ..', '             
            end     

            -----------------------------------------
            ------------------------------------ FUELED
            if t.components.perishable then
                str= str..' perishable=' .. t.components.perishable.perishtime ..', '
            end     

            -----------------------------------------
            ------------------------------------ FUELED
            if t:HasTag("lightbattery") then
                str= str..' lightbattery=true,'
            end     
            ...
            for recipeprefab,recipe in pairs(AllRecipes)do
                if t.prefab == recipeprefab then
                    ...
                    if recipe.builder_tag then
                        ...
                        if character then
                            str= str..' craftingprefab="' .. character ..'", '
                        end
            ...
            if t:HasTag("shadow_aligned") then
                str = str..'shadow_aligned=true,'
            end
            if t:HasTag("lunar_aligned") then
                str = str..'lunar_aligned=true,'
            end
            str=str..'},'
